...
class _MemberDetailBodyState extends ConsumerState<_MemberDetailBody> {
  ...
  Future<void> _topUpBalance() async {
    final m = widget.member;
    final isMinuteBased = (m.sessionDuration ?? 0) <= 0 &&
        (m.totalMinutes ?? 0) > 0 &&
        (m.totalSessions ?? 0) == 0;
    final unitIsSession = !isMinuteBased;
    ...
    final added = await showDialog<int>(
      ...
      builder: (_) => AlertDialog(
        backgroundColor: AppColors.card,
        title: Text(unitIsSession ? 'Top-up Sesi' : 'Top-up Menit',
            style: const TextStyle(color: AppColors.textPrimary)),
        content: Form(
          key: formKey,
          child: Column(
            ...
            children: [
              Text(
                unitIsSession
                    ? 'Tambahkan kuota sesi untuk member ini.'
                    : 'Tambahkan kuota menit untuk member ini.',
                style: const TextStyle(
                    color: AppColors.textSecondary, fontSize: 13),
              ),
              const SizedBox(height: 12),
              TextFormField(
                ...
                autofocus: true,
                decoration: InputDecoration(
                  labelText: unitIsSession ? 'Jumlah sesi' : 'Jumlah menit',
                  suffixText: unitIsSession ? 'sesi' : 'menit',
                ),
                validator: (v) {
                  final n = int.tryParse((v ?? '').trim());
                  if (n == null || n <= 0) return 'Masukkan angka > 0';
                  if (n > 100000) return 'Nilai terlalu besar';
                  return null;
                },
              ),
            ],
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              if (formKey.currentState?.validate() ?? false) {
                Navigator.pop(
                    context, int.parse(amountController.text.trim()));
              }
            },
    ...
    try {
      final sessionDuration = m.sessionDuration ?? 0;
      final newTotalMinutes = (m.totalMinutes ?? 0) +
          (unitIsSession && sessionDuration > 0
              ? added * sessionDuration
              : added);

      await ref.read(apiRepositoryProvider).updateMember(m.id, {
        'totalMinutes': newTotalMinutes,
        if (sessionDuration > 0) 'sessionDuration': sessionDuration,
      });
      widget.onRefresh();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(unitIsSession
                  ? 'Berhasil menambah $added sesi'
                  : 'Berhasil menambah $added menit')),
        );
      }
    } catch (e) {
      if (mounted) {
